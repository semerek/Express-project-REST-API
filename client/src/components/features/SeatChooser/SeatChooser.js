...
class SeatChooser extends React.Component {
  ...
  componentDidMount() {
    ...
    this.socket = io(process.env.NODE_ENV === 'production' ? '' : 'localhost:8000');
  ...
  render() {
    ...
    const freeSeats = seatsTotal -occupiedSeats.length;

    return (
      <div>
        <h3>Pick a seat</h3>
      <h3>Free seats: {freeSeats}/{seatsTotal}</h3>
        ...
        { (requests['LOAD_SEATS'] && requests['LOAD_SEATS'].pending) && <Progress animated color="primary" value={seatsTotal} /> }
